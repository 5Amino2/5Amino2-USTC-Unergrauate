...
void revise(int userid){//日程修改
	...
	for(int i=0;i<schedulenumbernow();i++){
		if(i==idin){
			fprintf(write,"%d %d %d %d %d %s %s\n",idin,userid,yearin,monthin,dayin,placein,tagin);
			continue;
		}
		fprintf(write,"%d %d %d %d %d %s %s\n",all[i].id-j,all[i].infomation.userid,all[i].date.year,all[i].date.month,all[i].date.day,all[i].place,all[i].tag);
	}
	fclose(write);
	return;
}
void userdelete(int userid){//日程删除
	...
	for(int i=0;i<n-1;i++){
		fprintf(write,"%d %d %d %d %d %s %s\n",all[i].id-(i>=idin),all[i].infomation.userid,all[i].date.year,all[i].date.month,all[i].date.day,all[i].place,all[i].tag);
	}
	fclose(write);
	return;
}
